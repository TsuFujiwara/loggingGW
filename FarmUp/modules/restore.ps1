#PCのアップロードファイルをリモートマシン（建物GW）に設置（その際、建物GW側のresotre_files内ファイルは無くなる）
...
try
{
    ...
    try
    {
        ...
        Write-Host $f1
        $f2 = "restore_files\bacnetd"
        $f=$f1.trim() + $f2.trim()
        Write-Host $f
        $transferResult = $session.PutFiles($f,"/usr/local/bin/", $TRUE, $transferOptions)
